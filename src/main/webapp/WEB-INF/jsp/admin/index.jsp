...
<body class="fixed-sidebar full-height-layout gray-bg" style="overflow: hidden">
	...
	<!-- 自定义js -->
	<script src="<%=path %>/js/hAdmin.js?v=4.1.1"></script>
...
$(function() {
	$.ajax({
		type : "POST",
		url : "<%=path%>/menu/treeMenu.do",
		dataType : "json",
		error : function(error) {
		},
		success : function(result) {
			showall(result.data,"side-menu");
			initClick();
		},
	});
});
...
function showall(menu_list,parent) {
	for (var menu in menu_list) {
		var dynamicId = "homeMenu" + menu_list[menu].id;
		if(menu_list[menu].hrefType === "1") {
			$("<li class='hidden-folded padder m-t m-b-sm text-muted text-xs'><span class='ng-scope'>"+menu_list[menu].name+"</span></li>").appendTo("#"+parent);
			dynamicId = parent;
		} else if(menu_list[menu].hrefType === "2") {
			$("<li><a href='#'><i class='fa fa-sitemap'></i><span class='nav-label'>"+menu_list[menu].name+"</span><span class='fa arrow'></span></a><ul id="+dynamicId+" class='nav nav-second-level collapse'></ul></li>").appendTo("#"+parent);
		} else if(menu_list[menu].hrefType === "3") {
			$("<li><a target='J_iframe' class='J_menuItem' href='<%=path%>/"+menu_list[menu].href+"'>"+menu_list[menu].name+"</a></li>").appendTo("#"+parent);
		}
		if(menu_list[menu].detail.length >0) {
			showall(menu_list[menu].detail,dynamicId);
		}
	}
}
